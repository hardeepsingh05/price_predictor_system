...
def model_evaluator_step(
    ...
    y_test (pd.DataFrame): The test data labels/target feature.
    ...
    X_test_preproessed = trained_model.named_steps["preprocessor"].transform(X_test)
    ...
    evaluation_metrics = evaluator.evaluate(
        trained_model.named_steps["model"], X_test_preproessed, y_test
    )

    # Ensure that the evaluation metrics are returned as a dictionary
    if not isinstance(evaluation_metrics, dict):
        raise ValueError("Evaluation metrics must be a returned as a dictionary.")
